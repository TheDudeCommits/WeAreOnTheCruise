...
describe('PACE speed upgrades', () => {
  ...
  it('Momentum: sinkings stack a capped speed surge the HUD can read, which holds and then drains', () => {
    ...
    // Holds for `hold` s after the last sinking, then drains to nothing. The queued level-ups resolved inside sail()
    // may raise the Momentum rank (the card RNG depends on everything else in the sim), so allow the max-rank drain.
    const maxCap = PASSIVES.momentum.maxRank * PASSIVES.momentum.perRank.surge!;
    const evs = sail(sim, MOMENTUM.hold + maxCap / MOMENTUM.drain + 0.2);
