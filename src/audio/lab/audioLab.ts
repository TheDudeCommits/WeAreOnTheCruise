...
const EVENT_GROUPS: [string, [string, () => void][]][] = [
  ...
  ['Enemies (pad position)', [
    ...
    ['Mortar + whistle', () => { const p = at(); const id = nextId++; emit({ type: 'enemy-fired', source: 7, projectile: 'enemy-mortar', x: p.x * 2, z: p.z * 2, dirX: 0, dirZ: 1, count: 1 }, { type: 'telegraph', id, shape: 'circle', team: 'enemy', x: p.x, z: p.z, radius: 14, duration: 2.2 }); later(2200, { type: 'explosion', x: p.x, z: p.z, radius: 14, kind: 'mortar', team: 'enemy' }); }],
    ['Elite spawn', () => { const p = at(); emit({ type: 'enemy-spawned', id: nextId++, defId: 'frigate', x: p.x, z: p.z, elite: true }); }],
    ['Kill brig', () => { const p = at(); const id = nextId++; emit({ type: 'enemy-killed', id, defId: 'brig', x: p.x, z: p.z, elite: false }); later(3000, { type: 'enemy-sunk', id, x: p.x, z: p.z }); }],
