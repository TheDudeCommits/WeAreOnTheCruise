...
import { blastCaptains, hurtCaptain } from '../captains-damage';
...
/** Damages the player, AI captains and every ship touching the circle. Returns the number of enemy ships hit. */
export function blastCircle(c: SimContext, rt: EventRuntime, x: number, z: number, radius: number, playerDamage: number, shipDamage: number, o: BlastOpts = NO_BLAST): number {
  ...
  if (playerDamage > 0) blastCaptains(c, x, z, radius, playerDamage);
...
export function blastLine(c: SimContext, rt: EventRuntime, x: number, z: number, angle: number, length: number, halfWidth: number,
  ...
  // AI captains caught in the strip take the same hit.
  if (playerDamage > 0) for (const k of c.state.captains) {
    if (!k.alive) continue;
    const rx = k.x - x, rz = k.z - z;
    const along = rx * dx + rz * dz, lat = Math.abs(rx * dz - rz * dx);
    if (along > -k.beam && along < length + k.beam && lat < halfWidth + k.beam * 0.6) hurtCaptain(c, k, playerDamage);
  }
  if (shipDamage <= 0) return 0;
