...
export const SPAWN_BANDS: readonly SpawnBand[] = [
  ...
  { from: 1, entries: [E('skiff', 8, 3, 6), E('cutter', 4, 1, 2)] },
  { from: 2, entries: [E('skiff', 7, 4, 6), E('cutter', 4, 1, 3), E('brig', 3, 1, 1), E('wraith', 4, 1, 2)] },
  { from: 3, entries: [E('skiff', 6, 4, 7), E('cutter', 4, 2, 3), E('brig', 3, 1, 2), E('corsair-brig', 3, 1, 2), E('fireship', 2, 1, 2), E('wraith', 4, 1, 2)] },
  { from: 4, entries: [E('skiff', 6, 4, 7), E('cutter', 3, 2, 3), E('brig', 4, 1, 2), E('corsair-brig', 3, 1, 2), E('fireship', 2, 1, 3), E('mortar-barge', 2, 1, 1), E('wraith', 4, 1, 3)] },
...
export const DIRECTOR = {
  ...
  budgetRate: (minute: number): number => 0.35 + 0.12 * minute + 0.012 * minute * minute,
  ...
  minAlive: (minute: number): number => Math.min(78, 4 + 5 * minute),
  ...
  hpScale: (heat: number): number => 1 + (heat - 1) * 0.6,
  damageScale: (heat: number): number => 1 + (heat - 1) * 0.25,
...
export const DIRECTOR_EVENTS: Readonly<Record<DirectorEventId, DirectorEventDef>> = {
  ...
  'storm-front': { id: 'storm-front', name: 'Storm Front', text: 'A storm front rolls in: rogue waves and lightning!', from: 2, weight: 6, maxPerRun: 5, duration: 24 },
...
export const EVENT_TUNING = {
  ambushSkiffs: (minute: number): number => Math.min(22, 10 + Math.floor(minute)),
  ambushRadius: 230,
