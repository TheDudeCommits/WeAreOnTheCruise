...
The selected style is Cinematic Anime A. Eight original Blender-authored hull GLBs are loaded from `public/assets/ships/`; the ninth ship, Polar Tang, remains procedural. Sails, figureheads, fittings, crew rigs and damage layers are procedural game geometry. `ShipHullAssets` loads the exported hull kits; the ship factory assembles the presentation and its LODs. Imported hull origin, scale and orientation are recorded in `hull-manifest.json`. Do not infer a collider from a presentation-only freeboard offset.

Imagegen-created limestone, painted timber and sky PNGs are loaded from `public/assets/materials/`. Cel materials, shared weather lighting, shadows, selective contours, ocean foam and FX implement the visual direction. The former full-scene normal/Sobel edge pass is not the active rendering path. World scenery is merged by material when the streamed records change.

`assets/source/` preserves raw GLBs and the provider catalog outside Vite public output. The official Sketchfab search inventory contains 1,676 records and 28 ship candidates, but zero model bytes were downloaded because authenticated official download was unavailable. Original Blender hulls must not be described as acquired Sketchfab meshes. Follow [ASSET-LICENSES.md](./ASSET-LICENSES.md) for provenance and license records before integrating later downloads.
