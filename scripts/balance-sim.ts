...
import type { CardOffer, MetaProfile, WeaponSlot } from '../src/game/types';
...
const CHECKPOINTS = [1, 5, 10, 12, 15] as const;
...
class Bot {
  ...
  drive(): void {
    ...
    const wantFull = urgent >= 1 || nearestD > 170 || (hpFrac < 0.3 && near > 2);
  ...
  pickCard(offers: readonly CardOffer[]): number {
    ...
    offers.forEach((o, i) => {
      ...
      switch (o.kind) {
        ...
        case 'new-weapon': score = PROXY === 'off' && this.proxied.size > 0 ? 20 : p.weapons.length < 4 ? 68 : 52; break;
        case 'passive-rank': score = (PASSIVE_PRIORITY[o.id] ?? 30) + 4; break;
        case 'new-passive': score = PASSIVE_PRIORITY[o.id] ?? 30; break;
  ...
  private proxyFire(slot: WeaponSlot): void {
    ...
    const eff = def.tags.includes('contact') ? 0.3 : 0.55;
    let dps = (lvl.damage * (lvl.count + extraAmount(st))) / (lvl.cooldown * cooldownMul(st)) * damageMul(st) * eff * (area ? 1.8 : 1);
...
function runOne(ship: ShipId, sea: SeaId, seed: string): RunReport {
  const t0 = Date.now();
  const world = new IslandField(seed);
  const sim = new Sim({ seed, shipId: ship, seaId: sea, meta: metaProfile(), world });
  ...
  while (sim.state.time < limit) {
    ...
    for (const e of sim.drainEvents()) {
      if (e.type === 'boss-spawned') bosses.push({ boss: e.boss, spawned: sim.state.time, killed: null });
...
console.log('sea               ship             seed  L@1 L@5 L@10 L@15 | kills@5/10/15   | alive@5/10/12max/15 | hp%@5/10/15   | died   | warden tidewyrm sovereign (s)  | ◈    | outcome');
for (const sea of SEAS) {
  for (const ship of SHIPS) {
    for (let i = 0; i < SEEDS; i++) {
      ...
      console.log(
        `${sea.padEnd(17)} ${ship.padEnd(16)} ${seed.padEnd(5)} ${L(1)} ${L(5)} ${L(10)} ${L(15)} | ${String(`${K(5)}/${K(10)}/${K(15)}`).padEnd(15)} | ${String(`${A(5)}/${A(10)}/${r.maxAlive12}/${A(15)}`).padEnd(19)} | ${String(`${H(5)}/${H(10)}/${H(15)}`).padEnd(13)} | ${fmtTime(r.died).padEnd(6)} | ${pad(boss('iron-warden'), 6)} ${pad(boss('tidewyrm'), 8)} ${pad(boss('sovereign'), 9)}      | ${pad(r.doubloons, 4)} | ${r.outcome}${r.revives ? ` (+${r.revives} revive)` : ''}`,
      );
      if (VERBOSE) console.log(`   loadout ${r.loadout.join(' ')} proxied=[${r.proxied.join(',')}] dmg=${JSON.stringify(r.damageByWeapon)} bot=${JSON.stringify(r.bot)} ${r.ms}ms`);
    }
  }
}
